...
def get_open_port():
    ...
    tree = parse("{}/nmap_log.txt".format(LOG_DIR))
    nmap = tree.getroot()
...
def get_information():
    ...
    for line in command(
        "egrep -r 'soap:Envelope' ./{} | grep -P 'hnap/[A-Za-z0-9]+.xml'".format(IID)
    ):
    ...
    for filepath in command('egrep -ril "action=" ./{}'.format(IID)):
        ...
        # .asp, .aspx (dlink, netgear)
            ...
            response, status_code = send_http_dummy(
                port=80,
                uri="/" + filepath.split("/")[-1],
                method="get",
                need_login=True,
            )
            if status_code == 200:
        ...
        for form_tag in soup.find_all("form"):
            ...
            if param_map:
                ...
                for m, p in action_map[action]:
                    if p.get("html_response_return_page", "false") == param_map.get(
...
def get_execve_log(dummy_length):
    execve_list = []
    for line in command(
        'strings {}/qemu.final.serial.log | grep "\[ANALYZE\]" | grep -i d34d'.format(
            WORK_DIR
        )
    ):
...
def send_hnap_dummy(port, hnap, data="", header="", need_login=False):
    ...
    if need_login:
        ...
        if LOGIN_TYPE == "dlink_hnap":
            cookies = SESSION
            headers["HNAP_AUTH"] = login.HNAP_AUTH(
                hnap.split("/")[-1], login.PRIVATE_KEY
            )
        else:
            req = SESSION
    headers[
...
def send_http_dummy(port, uri, method, headers={}, params={}, need_login=False):
    ...
    if (
        data.find(b"leave this field blank") != -1
        or data.find(b"user_page.asp") != -1
        or data.find(b"location.replace('login.htm')") != -1
        or data.find(b"login.ccp") != -1
    ):
...
def fuzz(dummy_info):
    ...
    if dummy_info[0] == "soap_param":
        ...
        for payload in PAYLOAD_LIST:
            response, status_code = send_hnap_dummy(
                port=dummy_info[1],
                hnap=dummy_info[2],
                data=dummy_info[3].format(payload=payload),
                need_login=dummy_info[4],
            )
            if (
                response.find(b">OK<") == -1
                and response.find(b">SUCCESS<") == -1
                and response.find(b">REBOOT<") == -1
            ):
                time.sleep(10)
                continue
            print("[+] ok")
        ...
        for payload in PAYLOAD_LIST:
            send_http_dummy(
                ...
                method="get",
                uri="/",
                headers={"Cookie": dummy_info[2] + payload},
            )
    ...
    for info in infos:
        ...
        if port in [80, 8181, 8080]:
            for hnap in hnap_map:
                ...
                if hnap in [
                    "/HNAP1/Reboot",
                    "/HNAP1/SetInternetProfileAlpha",
                    "/HNAP1/SetUSBStorageSettings",
                ]:
                    continue
                ...
                for data in data_list:
                    response, status_code = send_hnap_dummy(
                        port=port, hnap=hnap, data=data
                    )
                    print(
                        "[*] send {} signature without login {}".format(
                            hnap, datetime.datetime.now()
                        )
                    )
                    need_login = status_code == 401
                    if need_login:
                        response, status_code = send_hnap_dummy(
                            port=port, hnap=hnap, data=data, need_login=True
                        )
                        print(
                            "[*] send {} signature with login {}".format(
                                hnap, datetime.datetime.now()
                            )
                        )
                        ...
                        if (
                            response.find(b">OK<") != -1
                            or response.find(b">SUCCESS<") != -1
                            or response.find(b">REBOOT<") != -1
                        ):
                            time.sleep(9)
                    ...
                    dummy_idx_list.append(
                        [
                            "hnap_data",
                            port,
                            hnap,
                            data.replace("d34d" + str(idx), "{payload}"),
                            need_login,
                        ]
                    )
                    idx += 1
            ...
            for uri in action_map:
                ...
                for method, params in action_map[uri]:
                    ...
                    for key in params:
                        ...
                        response, status_code = send_http_dummy(
                            port=port, method=method, uri=uri, params=params
                        )
                        ...
                        time.sleep(1)
                        dummy_idx_list.append(
                            ["action", port, method, uri, params, key, need_login]
                        )
                        params[key] = original
                        idx += 1
